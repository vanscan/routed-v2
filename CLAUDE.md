...
- **`kotlinVersion` in `app.json` is load-bearing — it must equal React Native's Kotlin pin**: The real Kotlin compiler on the Android build comes from React Native's gradle plugin (`node_modules/react-native/gradle/libs.versions.toml` — **2.1.20** for RN 0.81), and `android.kotlinVersion` cannot change that. What the pin DOES feed (via `plugins/withKotlinVersion.js` → `rootProject.ext.kotlinVersion`) is the KSP version derivation in both Expo's KSPLookup and async-storage's `config.gradle`. If the pin ≠ RN's compiler version, the derived KSP is incompatible and the EAS Android build dies in `:react-native-async-storage_async-storage:kspReleaseKotlin` ("ksp-X is too new for kotlin-Y" or an internal compiler error). This has broken repeatedly (commits 7e0ff83, 70a8ca4, cd59390, PR #83 — each pinned a different wrong value). When upgrading Expo SDK / React Native, check RN's `libs.versions.toml` and update both `app.json` and the fallback in `withKotlinVersion.js` to that exact version.
